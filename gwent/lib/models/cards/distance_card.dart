...
class DistanceCard implements AbstractUnitCard{
  final String _name;
  int _strength;
  final UnitEffect _effect;
  AbstractCardState _state = ClearState();

  DistanceCard(this._name, this._strength, this._effect){
    _state.setCard(this);
  }
  ...
  void applyBondEffect(BoardSection boardSection) {
    final List<DistanceCard> cards = boardSection.getDistance();
    bool found = false;
    for (DistanceCard card in cards) {
    ...
    if (found) {
      doubleStrength();
      boardSection.applyBondEffectDistance(this);
    }
  }

  /// applies the moral effect to all the cards in the distance section
  @override
